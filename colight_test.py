import gym
from agent.colight.CBEngine_round3 import CBEngine_round3 as CBEngine_rllib_class
...
class RLlibTFCheckpointPolicy():
    def __init__(
        self, load_path, algorithm, policy_name, observation_space, action_space, num_agents
    ):
        ...
        if self._algorithm == "PPO":
            ...
            from ray.rllib.agents.dqn.dqn_torch_policy import DQNTorchPolicy as LoadPolicy

            if args.algorithm == "APEX":
                print("IF APEX")
                from ray.rllib.agents.dqn.apex import APEX_DEFAULT_CONFIG
                self.config = {'model': deepcopy(APEX_DEFAULT_CONFIG['model'])}
                self.config['model'].update({
                    'custom_model': "colight",
                    'custom_model_config': {
                        'num_neighbors': self._num_agents,
        ...
        print(self.config)
        self.policy = LoadPolicy(flat_obs_space, self._action_space, self.config)
...
def get_agent(target_iteration,algorithm,foldername,num_agents):
    ACTION_SPACE = gym.spaces.Discrete(9)
    for dirpath, dirnames, file_names in os.walk('agent/colight'):
    ...
    if algorithm == 'APEX':
        OBSERVATION_SPACE = gym.spaces.Dict({
            "observation": gym.spaces.Box(low=-1e10, high=1e10, shape=(num_agents, obs_size,), dtype=np.float32),
            'adj': gym.spaces.Box(low=-1e10, high=1e10, shape=(num_agents, num_agents, num_agents), dtype=np.float32)
        })
    else:
        OBSERVATION_SPACE = gym.spaces.Dict({
            ...
            agents.append(
                (
                    RLlibTFCheckpointPolicy(load_path=model_path, algorithm=algorithm, policy_name="default_policy",
                                            observation_space=OBSERVATION_SPACE, action_space=ACTION_SPACE, num_agents=num_agents)
                    , iteration
                )
            )
    return agents
...
if __name__ == "__main__":
    ...
    if(args.algorithm not in ['FT','MP']):
        agents = get_agent(target_iteration=args.iteration, algorithm=args.algorithm, foldername = args.foldername, num_agents=args.agents)
