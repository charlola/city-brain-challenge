...
if __name__ == "__main__":
    ...
    class MultiFlowCBEngine(CBEngine_rllib_class):
        def __init__(self, env_config):
            env_config["simulator_cfg_file"] = simulator_cfg_files[
                (env_config.worker_index - 1) % len(simulator_cfg_files)]
    ...
    DQNTrainer = GenericOffPolicyTrainer.with_updates(
        name="DQN", default_policy=DQNTorchPolicy, default_config=DEFAULT_CONFIG)

    # train model
    ray.tune.run(DQNTrainer, config=tune_config, local_dir=local_path, stop=stop,
