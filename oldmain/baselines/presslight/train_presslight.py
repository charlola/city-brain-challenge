...
from hp_config import rt_hp_config
import sys
import time
import numpy as np
...
def train(agent_spec, simulator_cfg_file, gym_cfg, metric_period, config):
    logger.info("\n")
    logger.info("*" * 40)
    ...
    import gym
    import CBEngine.envs
    gym.logger.setLevel(gym.logger.ERROR)

    env = CBEngine.envs.CBEngine(simulator_cfg_file=simulator_cfg_file,
                                 thread_num=1,
    ...
    agent.set_hyperparameters(config)
    ...
    env.set_log(0)
    env.set_warning(0)
    env.set_ui(0)  # throws error
    env.set_info(0)  # throws error
    ...
    for e in range(args.episodes):
        ...
        mlflow.log_metric("avg_travel_time", env.eng.get_average_travel_time(), e)
        mlflow.log_metric('mean_eps_reward', (episodes_rewards[agent_id] / episodes_decision_num), e)
...
def trainable(config):
    ...
    metrics = {}
    metrics["total_served_v"] = result['data']['total_served_vehicles']
    metrics["delay_index"] = result['data']['delay_index']
    mlflow.log_metrics(metrics)

    tune.report(score=metrics['total_served_v'])
...
parser.add_argument('--save_dir', type=str, default="model/presslight",  # /dqn_warm_up
                    help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="cmd_log/presslight",
...
analysis = tune.run(
    trainable,
    config=rt_hp_config
    #scheduler=MedianStoppingRule(time_attr="episodes",
                                 #metric="score",
                                 #grace_period=20,
                                 #min_samples_required=5,
                                 #mode="min", hard_stop=True,
                                 #min_time_slice=20),
    #stop=EarlyStopping(metric="score", mode="min", patience=20)
)

df = analysis.results_df
logger.info("Best config:{}".format(analysis.get_best_config(metric="score", mode='min')))
